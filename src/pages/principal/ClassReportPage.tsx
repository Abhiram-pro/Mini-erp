...
import {
  ...
  getDay, subMonths, addMonths, isToday, parseISO,
} from 'date-fns';
import { db } from '../../firebase/config';
import type { AttendanceRecord, Student } from '../../types';

export default function PrincipalClassReportPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([classSnap, studentSnaps]) => {
      if (classSnap.exists()) setClassName(classSnap.data().name);
      setStudents(studentSnaps.docs.map((d) => ({ id: d.id, ...d.data() } as Student)));
    });
  }, [classId]);
  ...
  useEffect(() => {
    ...
    getDocs(query(collection(db, 'attendance'), where('classId', '==', classId))).then((snaps) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/principal')} className="text-slate-500 hover:text-slate-700 text-sm font-medium">
          ← Back
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-slate-800">{className || 'Class'} — Report</h1>
          <p className="text-slate-500 text-sm">Full attendance history</p>
        </div>
      </div>
              ...
              return (
                <button
                  ...
                  className={`aspect-square flex flex-col items-center justify-center rounded-lg text-xs font-medium transition-colors
                    ${future || !record ? 'text-slate-300 cursor-default' : 'hover:bg-primary-50 cursor-pointer text-slate-700'}
            ...
            {Object.entries(selected.entries)
              ...
              .map(([studentId, status]) => (
                <div key={studentId} className="flex items-center justify-between py-2">
                  <span className="text-sm text-slate-700">{studentMap[studentId] ?? '—'}</span>
